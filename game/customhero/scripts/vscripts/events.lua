...
function GameMode:OnEntityKilled( keys )
  ...
  --##############NEUTRAL ISLAND BOSS TEAM BOUNTY#######################
  if killedUnit:GetUnitName() == "neutral_island_boss" then
    ...
    if team == 2 then
      teamname = "The Radiant"
    end
    if team == 1 then
      teamname = "The Dire"
    end

    GameRules:SendCustomMessage(teamname.." has killed the NEUTRAL ISLAND BOSS and earned "..gold.." gold each.", 4, 1)

    for i=1,table.getn(heroes) do
      if heroes[i]:GetTeam() == team then
        print(heroes[i]:GetUnitName())
...
-- Handing out loot from a killed entity
